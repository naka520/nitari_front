...
//   const apiURL = "https://func-backend.azurewebsites.net/api/diary/week";
...
//         <div key={idx} className="bg-white shadow-lg rounded p-4 mb-4 w-full max-w-md">
//           <div className="flex items-center space-x-4">
//             <img className="w-32 h-32 object-cover rounded" src={report.imageUrl} alt={report.title} /> {/* imageUrlフィールドを使用 */}
//             <div className="ml-12">
//               <h3 className="font-bold text-xl mb-2">{report.title}</h3> {/* titleフィールドを使用 */}
//               <p className="text-gray-600 mb-2 ">{report.date}</p> {/* dateフィールドを使用 */}
//             </div>
//           </div>
//           <p className="text-sm mt-2">{report.description}</p> {/* descriptionフィールドを使用 */}
//         </div>
//       ))}
//     </div>
//   </div>
  ...
  useEffect(() => {
    const initLiff = async () => {
      try {
        ...
        try {
          const response = await axios.get(`${apiURL}?userId=${profile.userId}`);
          setData(response.data);
        } catch (err) {
          console.error("Failed to fetch data: ", err);
        }
    ...
    <div className="BackColor flex flex-col min-h-screen h-auto w-screen">
      ...
      <div className="flex flex-col items-center">
        {data.map((report, idx) => (
          <div key={idx} className="bg-white shadow-lg rounded p-4 mb-4 w-full max-w-md">
            <div className="flex items-center space-x-4">
              <img className="w-32 h-32 object-cover rounded" src={report.imageUrl} alt={report.title} />
              <div className="ml-12">
                <h3 className="font-bold text-xl mb-2">{report.title}</h3>
                <p className="text-gray-600 mb-2 ">{report.date}</p>
              </div>
